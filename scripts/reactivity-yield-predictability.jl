...
function full_exp_reactivity(eta_i, com)
    S = richness(com)
    d = S - 1
    A = remove_diagonal(com.A)
    eta = equilibrium_abundance(com)
...
Threads.@threads for community_idx in 1:n_communities
    ...
    expected_reactivity_sp(i) =
        sqrt(sum(remove_diagonal(A)[i, :] .^ 2) / (S - 1)) *
        norm(yield) *
        sqrt(1 - yield[i] / norm(yield))
...
processed_df = combine(
    groupby(df, [:reactivity, :yield, :intensity, :expected_reactivity, :community_id]),
...
with_theme(p_theme) do
    ...
    for (i, sdf) in enumerate(groupby(processed_df, :community_id))
        ...
        exp_reactivity_full = sqrt.(full_exp_reactivity.(eta, Ref(com)))
        scatter!(eta_eq, reactivity; color = :grey, markersize, strokewidth)
        # lines!(eta, exp_reactivity2; color = :grey, label = "approx")
        lines!(eta, exp_reactivity_full; color = :coral, label = "expectation", linewidth = 2, alpha = 0.7)
    end
    axislegend()
    ...
    for ((marker_idx, intensity), sdf) in
        ...
        errorbars!(sdf.reactivity, sdf.predictability, sdf.predictability_error, linewidth = 1, whiskerwidth = 3)
        scatter!(
            sdf.reactivity,
            sdf.predictability;
