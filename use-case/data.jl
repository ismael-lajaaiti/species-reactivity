...
remove_diagonal(matrix) = matrix - Diagonal(matrix)
...
with_theme(p_theme) do
    ...
    for (eta, com) in zip(eta_communities, communities)
        ...
        exp_reactivity = sqrt.(full_exp_reactivity.(eta_linrange, Ref(com)))
